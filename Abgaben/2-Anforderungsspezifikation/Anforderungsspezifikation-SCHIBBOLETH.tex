...
\subsection{Anwendungsfälle (Tobias und Daniel)}
  ...
  \begin{figure}
  	...
  	\textwidth]{QuizzLoginRegister.png} \label{pic:Startseite} 
  \end{figure}
  ...
  \begin{table}
  	[H] \label{7} 
  	\begin{tabular}
  		{|l|p{10cm}|} \hline \textbf{7} & \textbf{Neues Spiel} \\
  		\hline \textbf{Akteure} & Paul Neuling, Klaus Zocker\\
  ...
  \begin{table}
  	[H] \label{8} 
  	\begin{tabular}
  		{|l|p{10cm}|} \hline \textbf{8} & \textbf{Gegner herausfordern} \\
  		\hline \textbf{Akteure} & Paul Neuling, Klaus Zocker\\
  		\hline \textbf{Ziel} & Der Akteur möchte einen anderen Spieler herausfordern\\
  		\hline \textbf{Vorbedingungen} & Der Akteur hat bereits ein neues Spiel gestartet und eine Liste ist erschien(Anwendungsfall 7) \\
  		\hline \textbf{Regulärer Ablauf} & 1. Der Akteur drückt in der Liste auf einen Spieler und fordert ihn heraus\\
  		\hline \textbf{Varianten} & keine \\
  		\hline \textbf{Nachbedingungen} & Es wird eine Anfrage an den Spieler gesendet\\
  		\hline \textbf{Fehler-/Ausnahmefälle} & Leere Liste, wenn keine Spieler online sind \\
  		\hline 
  	\end{tabular}
  \end{table}
...
\subsubsection{Offline-Modus starten()}
\begin{itemize}
...
\subsubsection{Online-Modus starten()}
\begin{itemize}
...
\subsubsection{Registrieren(nutzername, passwort)}
\begin{itemize}
\item Der Nutzer initiiert die Registrierung mit einem Buttondruck und gibt seine Daten, mit denen er sich registrieren möchte, in zwei Eingabefelder ein und bestätigt die Eingabe per Button.
...
\subsubsection{Login(nutzername, passwort)}
\begin{itemize}
...
\item \textbf{Vorbedingung}: Der Nutzer ist eingeloggt.
...
\subsubsection{Neues Spiel starten(modus)}
\begin{itemize}
...
\subsubsection{Gegner herausfordern(nutzer)}
\begin{itemize}
